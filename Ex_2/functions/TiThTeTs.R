TiThTeTs <- function(Dat)
  {
    ## Generate a new object of class ctsm
    model <- ctsm$new()
    ## Add a system equation and thereby also a state
    model$addSystem(dTs ~ ( 1/(Cs*Ris)*(Ti-Ts) )*dt + exp(p11)*dw1 )
    model$addSystem(dTi ~ ( 1/(Ci*Ris)*(Ts-Ti) + 1/(Ci*Rih)*(Th-Ti) + 1/(Ci*Rie)*(Te-Ti) + Aw/Ci*Ps  )*dt + exp(p22)*dw2 )
    ...
    model$setParameter(  Ts = c(init=10  ,lb=0     ,ub=25) )
    model$setParameter(  Ti = c(init=15  ,lb=-15   ,ub=25) )
    model$setParameter(  Th = c(init=10  ,lb=-15   ,ub=25) )
    model$setParameter(  Te = c(init=10  ,lb=-15   ,ub=25) )
