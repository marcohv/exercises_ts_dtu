...
title: \Large Excercise 4 - Estatistical models for forecasting in energy systems
...
*The forecast seems to match in some parts but it is not really good.*  
...
*These differences could be causd by the shadding of the chimmeny that appears in the late morning, differences with the orientations/tilt of the sensor.*

Now, define a model the relation between the solar power and the global radiation NWP is conditional on the time of day. Fit it, using base splines and lm() and rls(), and finally with a kernel model.
...
*Yes, specially when applying only the Linear Model (lm)  because it lacks the ability to adapt the variance with time*

\pagebreak

# Q7 - Wind forecasting

These are not real wind power measurements. The "wind power" Pw, which is used, is actually the observed wind speed put through a simple power curve function. Hence this is a "simplified" wind power, which has the basic properties of a "real" wind power – remember "real" wind power signals are potentially quite different depending on the wind turbine and wind farm properties, and surroundings etc. Hence, the signals used holds the basic properties of a wind power measurement.

```{r q7_ini, echo=FALSE, warning=FALSE, results='hide'}
...
```{r q7_dataframe, echo=FALSE, warning=FALSE, results='hide'}
## Make a data.frame with synced observations and NWPs
k <- 24
...
```{r q7_lm, echo=FALSE, warning=FALSE, results='hide'}
...
## The score
rmse_lm <- rmse(X$residuals_lm[itest])

# plot forecats
plotmulti(X[itest, ], c("Pw$|Pw_hat_lm"))
```
Look at the scatter plot of the wind power $P_w$ vs. wind speed NWP $W_s$ `(plot(X$Ws, X$Pw))` with the linear model fit `(abline(fit))`. 
  * Can you conclude that a linear model is suitable?
  
*No, a linear model is definitively not suitable in this case since the data doesn't follow a linear path (looks more like a logistic regression)*

### Spline model 

Making the model non-linear in the wind speed
```{r q7_spline, echo=FALSE, warning=FALSE, results='hide'}
...
## Improvements?
rmse_bslm <- rmse(X$residuals_bslm[itest])
...
*Yes, based on the graph of the forecasts and the actual wind power ($P_W$), the base spline model has a better fit than the linear regression model. Also, comparing the RMSE of `r round(rmse_lm, 3)` for the linear regression model to `r round(rmse_bslm, 3)`, there was an imporvement by uisng a non-linear relationship.*

### Base spline model with rls
```{r q7_spline_rls, echo=FALSE, warning=FALSE, results='hide'}
## Base spline model with rls
obj <- function(prm, frml, data, k, ieval = 1:nrow(data)) {
    print(prm)
    ## Apply a low-pass filter on the input
...
lambda_rls <- result$minimum

## Calculate the forecasts
fit <- rls(as.formula(frml), lambda = lambda_rls, X, k)
...
## Improvements
rmse_bsrls <- rmse(X$residuals_bsrls[itest])

```
  * Do we achieve improvements with RLS over LM?

*There was no improvement when comparing the RMSE of `r round(rmse_bslm, 3)` agaisnt the RMSE of the rls model `r round(rmse_bsrls, 3)`*

  * If not, can you give some explanation? (i.e. somehow weird, since RLS with $\lambda = 1$ give the same result as LM, right!?)...what was the optimal $\lambda$ found using the training set?

*The lack of improvements could come because the minimum lambda encountered thorugh the optimisation (`r round(lambda_rls, 3)` is very close to 1 which gives the same results to Lm, so no big improvement is achieved.*

### Kernel model

```{r q7_kernel, echo=FALSE, warning=FALSE, results='hide'}
...
## Improvements?
rmse_kn <- rmse(X$residuals_kn[itest])

#model comparison
Model_wind <- c("Base spline and lm","Kernel",
                "Base spline and rls","lm")
RMSE_wind <- c(rmse_bslm, rmse_kn, rmse_bsrls,  rmse_lm)
                     
data.frame(Model_wind, RMSE_wind)
```
  * Finally, with the kernel model, do we achieve improvements?
  
*The kernel model gives a very similar fit as the previous models but if we just consider the RMSE then the "Base spline with lm" is still the best option.*
...
Finally, an example of how a probabilistic forecasting model can be setup is presented. It is very simple: replace linear regression lm() with quantile regression rq(), and replace the RMSE score function with Continuous Ranked Probability Score (CRPS). 


>Every possible future has an assigned probability when doing probabilistic forecast. These forescasts however at not equally accurate and there is a need for evaluating them. There are simple metrics that asses the accurary such as MAE (Mean Absolute Error) or MAPE (Mean Absolute Percentage Error) but this are not suitable for probabilistic forecasts.  
The Continuous Ranked Probability Score (CRPS) generalizes the MAE to the case of probabilistic forecasts. It compares a full distribution with the observation, where both are represented as cumulative distribution functions (cdfs).  It is sensitive to the entire permissible range of the parameter of interest. It can be interpreted as an integral over all possible scores. For the case of a deterministic forecast, the CRPS is equal to the mean absolute error (MAE) and, therefore, has a clear interpretation (@gneiting2007strictly, @hersbach2000decomposition, @brown1974admissible).

```{r q8_ini, echo=FALSE, warning=FALSE, results='hide'}
## Init by deleting all variables and functions
rm(list=ls())
## Set the working directory
setwd(".")
...
Data <- readRDS("data_soenderborg.RDS")
```
```{r q8_data, echo=FALSE, warning=FALSE, results='hide'}
## Make a data.frame with synced observations and NWPs
k <- 24
...
```{r q8_quantile_reg, echo=FALSE, warning=FALSE, results='hide'}
...
crps_qr <- crpsDecomposition(tmp[ ,1],  tmp[ ,-1])$CRPS
```

### Base spline model with Quantile Regression 

```{r q8_quantile_spline, echo=FALSE, warning=FALSE, results='hide'}
...
crps_bsqr <- crpsDecomposition(tmp[ ,1],  tmp[ ,-1])$CRPS
```
  * Do we gain in predictive performace using the base spline model over the linear model?
  
*Yes, looking at the graphs we can see that the test-values fall more into the quantiles of the base spline model compared to the linear model. Also the CRPS improves from `r round(crps_qr, 3)` in the linear model to `r round(crps_bsqr, 3)` using base splines.*

### Kernel model with Quantile Regression 
...
```{r q8_quantile_kernel, echo=FALSE, warning=FALSE, results='hide'}

## What about a kernel model?
## Below wass pasted from "q6_solar_forecast.R":
## It was "converted" from the least squares into a quantile regression
## Needed to replace lm() with rq(), and rmse with crpsDecomposition()
...
obj <- function(h, frml, tau,  data, k = k, ieval = 1:nrow(data), n_min=10, return_yhat = FALSE){
    ...
    if(return_yhat){
        ...
        ## Calculate the score value to use as basis for evaluate and optimize 
            
            #nm is used to select the column of the dataset that is
              #the response variable "Y" in the regression formula Y ~ x
        nm <- all.vars(as.formula(frml))[1] 
        
            #data[ieval, nm] subsets the data to the evaluation interval
              # and selects only the column "nm" which is the response variable "Y"
        
            #yhat is a list of matrices that are binded by rows (rbind) 
...
```
* Do we gain in predictive performace using the kernel model?
  
*Again, when looking at the graphs we can see that the test-values fall more into the higher quantiles of the kernel model compared to the linear model. Also the CRPS improves from `r round(crps_qr, 3)` in the linear and `r round(crps_bsqr, 3)` using base splines, to `r round(crps_knqr, 3)` with the kernel model.*
