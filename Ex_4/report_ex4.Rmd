---
title: \Large Excercise 4 - Forecasting
...
In the exercise numerical weather predictions (NWPs) are used as model input. The
first step is to understand how they are set up. They are set up as matrices.
...
```
Try to learn how the data is setup, divide into a training and a test set, fit a linear regression model for k = 1 step ahead. 
...
## Make a scatter plot
#plot(Data$Ta, Data$Tanwp$k1) #not plotted in the report

## Also of the k = 24 hour forecast
#plot(Data$Ta, Data$Tanwp$k24) #not plotted in the report
...
*yes, the weather forecast seems reasonable even with a k = 24, it doesn't change too much and fits relatively good a line, altough the best is with k=1*

Try the laod forecast for a different house. Give an example and a summary of what you find.  

*For House 4, with k = 1 steps ahead, the load forescast is not so good and there is still alot of variation not being considered. Nevertheless the model is able to account for 49% of the variance (R^2) and has a low RMSE compared to the other houses.*
```{r q1_new_house, echo=FALSE, warning=FALSE, results='hide'}
...
In order to take dynamics into account we can filter the inupts. One can say it is a transformations of the inputs (like with base splines).

First in order to model a linear dynamical 1st order system (i.e. single RC) make a sequence like an on/off signal.
...
*The low-pass filter smoothness the signal and makes the sudden on/off changes slower. When a1 takes a value of 1 then the filtered signal is always 0. When a1 = 1, then the filtered signal follows the input exactly. The time constant is how long it takes to reach the 96% of the input signal.*
...
*It is tuned for the thermal mass of the buidling compared to the outdoor temperature but doesn't follow properly the fluctuations.*
...
Finally, include a diurnal curve using base splines. Make the base splines using bs(). Do we get better forecasts?
...
*Yes, including the splines for the diurnal curve improved the RMSE to `r round(rmse4, 3)`.*

\pagebreak

**(Optional)** How to choose the degrees of freedom df? maybe use AIC or BIC?
(Out of currect scope) Use Fourier series instead as basis functions for the diurnal curve.
...
*No,* **lm()** *can estimate the coefficientes of the individual series but when they are combined it can not find the parameters since it is not anymore a line.*  

Does rls()?   
*Yes, using* **rls()** *it is possible to estimate the parameters of the combined data.*

Try to change the forgetting factor $\lambda$. What happens when it is set to 1 compared to the results obtained from lm()?  

*The forgetting factor tells how much weight the model give to older data, $\lambda = 0$ means that the model "lives in the present" and ignores previous data; $\lambda = 1$ means that the model "remebers" everything. ASetting $\lambda = 1$ the last coefficients are very close to those obtained by lm().*
...
*Yes, the forgetting coefficient has to be chosen properly otherwise the model will not be fitted properly.*
...
*No, the load forecast does not seem to follow properly the actual load from the test set.*  
...
**$\lambda$** *was set to 0.99 but it doesn't seem to be optimal*
