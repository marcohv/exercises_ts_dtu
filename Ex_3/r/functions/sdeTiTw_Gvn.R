sdeTiTw_Gvn <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw-Ti) + gAInner/Ci*Gv + 1/Ci*Qi )*dt + exp(p11)*dw1)
    model$addSystem(dTw ~ ( 1/(Cw*Riw)*(Ti-Tw) + 1/(Cw*Rwe)*(Te-Tw) + gAOuter/Cw*Gouter )*dt + exp(p22)*dw2)
    ...
    model$setParameter(  gAOuter = c(init=0.5   ,lb=0.001 ,ub=10))
