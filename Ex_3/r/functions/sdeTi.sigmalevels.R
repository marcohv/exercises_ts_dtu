OsdeTi_sigmalevels <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Rie)*(Te-Ti) + Aw/Ci*Gv + 1/Ci*Qi )*dt + (1+(stepQi*sigmalevel))*exp(p11)*dw1)
    ...
    model$setParameter(  Ci = c(init=5E5 ,lb=1E4   ,ub=1E7) )
    model$setParameter( Rie = c(init=0.2 ,lb=1E-5  ,ub=10) )
    ...
    model$setParameter( sigmalevel= c(init=10  ,lb=0    ,ub=100) )
