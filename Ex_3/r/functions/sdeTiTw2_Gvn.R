sdeTiTw2_Gvn <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw1-Ti) + gAInner/Ci*Gv + 1/Ci*Qi )*dt + exp(p11)*dw1)
    model$addSystem(dTw1 ~ ( 1/(Cw1*Riw)*(Ti-Tw1) + 1/(Cw1*Rww)*(Tw2-Tw1) )*dt + exp(p22)*dw2) # Aw*(1-p)/Cw*Gv
    model$addSystem(dTw2 ~ ( 1/(Cw2*Rww)*(Tw1-Tw2) + 1/(Cw2*Rwe)*(Te-Tw2) + gAOuter/Cw2*Gouter )*dt + exp(p33)*dw3)
    ## Set the names of the inputs
    model$addInput(Te,Gv,Gouter,Qi)
    ...
    model$setParameter(  Tw2 = c(init=34  ,lb=10    ,ub=45) )
    ## Set the initial value of the parameters for the optimization
    model$setParameter(  Ci = c(init=1E5 ,lb=1E4   ,ub=1E7) )
    model$setParameter(  Cw1 = c(init=3E5 ,lb=1E4   ,ub=1E8) )
    model$setParameter(  Cw2 = c(init=5E5 ,lb=1E4   ,ub=1E8) )
    ...
    model$setParameter(  gAInner = c(init=0.5   ,lb=0.001 ,ub=10))
    model$setParameter(  gAOuter = c(init=0.5   ,lb=0.001 ,ub=100))
