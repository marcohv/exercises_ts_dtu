sdeTiTw_GinTw_splG3_sigmalevels <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw-Ti) + 1/Ci*Qi + p/Ci*(gA1*Gspl1+gA2*Gspl2+gA3*Gspl3) )*dt + (1+(stepQi*sigmalevel))*exp(p11)*dw1)
    model$addSystem(dTw ~ ( 1/(Cw*Riw)*(Ti-Tw) + 1/(Cw*Rwe)*(Te-Tw) + (1-p)/Cw*(gA1*Gspl1+gA2*Gspl2+gA3*Gspl3) )*dt + (1+(stepQi*sigmalevel))*exp(p22)*dw2)
