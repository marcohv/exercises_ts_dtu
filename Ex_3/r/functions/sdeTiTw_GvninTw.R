sdeTiTw_GvninTw <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw-Ti) + 1/Ci*Qi + Aw*p/Ci*Gv )*dt + exp(p11)*dw1)
    model$addSystem(dTw ~ ( 1/(Cw*Riw)*(Ti-Tw) + 1/(Cw*Rwe)*(Te-Tw) + Aw*(1-p)/Ci*Gv + Awall/Cw*Gvn.2 )*dt + exp(p22)*dw2)
    ## Set the names of the inputs
    model$addInput(Te,Gv,Gvn.2,Qi)
