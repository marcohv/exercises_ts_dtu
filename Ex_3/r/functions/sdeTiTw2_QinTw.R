sdeTiTw2_QinTw <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi  ~ ( 1/(Ci*Riw)*(Tw1-Ti) + Aw*p/Ci*Gv )*dt + exp(p11)*dw1)
    model$addSystem(dTw1 ~ ( 1/(Cw1*Riw)*(Ti-Tw1) + 1/Cw1*Qi + 1/(Cw1*Rww)*(Tw2-Tw1) + Aw*(1-p)/Cw1*Gv )*dt + exp(p22)*dw2) # Aw*(1-p)/Cw*Gv
    ...
    model$setParameter(  Tw1 = c(init=34  ,lb=20    ,ub=45) )
    model$setParameter(  Tw2 = c(init=34  ,lb=20    ,ub=45) )
    ...
    model$setParameter(  Cw2 = c(init=4E5 ,lb=1E4   ,ub=1E8) )
    model$setParameter( Riw = c(init=0.2 ,lb=1E-5  ,ub=10) )
    model$setParameter( Rww = c(init=0.2 ,lb=1E-5  ,ub=10) )
    model$setParameter( Rwe = c(init=0.1 ,lb=1E-5  ,ub=10) )
    model$setParameter(  Aw = c(init=0.2 ,lb=0.001 ,ub=10))
    model$setParameter(  p  = c(init=0.5 ,lb=0.001 ,ub=1 ))
