sdeTiTw2_splG3_sigmalevels <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi  ~ ( 1/(Ci*Riw)*(Tw1-Ti) + 1/Ci*Qi + 1/Ci*(gA1*Gspl1+gA2*Gspl2+gA3*Gspl3) )*dt + (1+(stepQi*sigmalevel))*exp(p11)*dw1)
    model$addSystem(dTw1 ~ ( 1/(Cw1*Riw)*(Ti-Tw1) + 1/(Cw1*Rww)*(Tw2-Tw1) )*dt + (1+(stepQi*sigmalevel))*exp(p22)*dw2) # Aw*(1-p)/Cw*Gv
    model$addSystem(dTw2 ~ ( 1/(Cw2*Rww)*(Tw1-Tw2) + 1/(Cw2*Rwe)*(Te-Tw2) + 1/Cw2*(gA1*Gspl1+gA2*Gspl2+gA3*Gspl3) )*dt + (1+(stepQi*sigmalevel))*exp(p33)*dw3)
    ## Set the names of the inputs
    model$addInput(Te,Gspl1,Gspl2,Gspl3,Qi,stepQi)
    ...
    model$setParameter(  Tw2 = c(init=34  ,lb=10    ,ub=500) )
    ...
    model$setParameter(  Cw2 = c(init=1E5 ,lb=1E2   ,ub=1E8) )
    ...
    model$setParameter(  gA1 = c(init=0.2 ,lb=0.001 ,ub=1))
    model$setParameter(  gA2 = c(init=0.1 ,lb=0.001 ,ub=1))
    model$setParameter(  gA3 = c(init=0.2 ,lb=0.001 ,ub=1))
