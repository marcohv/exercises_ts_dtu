sdeTiTw_GinTw_splG5_sigmalevels <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw-Ti) + 1/Ci*Qi + p/Ci*(gA1*Gspl1+gA2*Gspl2+gA3*Gspl3+gA4*Gspl4+gA5*Gspl5) )*dt + (1+(stepQi*sigmalevel))*exp(p11)*dw1)
    model$addSystem(dTw ~ ( 1/(Cw*Riw)*(Ti-Tw) + 1/(Cw*Rwe)*(Te-Tw) + (1-p)/Cw*(gA1*Gspl1+gA2*Gspl2+gA3*Gspl3+gA4*Gspl4+gA5*Gspl5) )*dt + (1+(stepQi*sigmalevel))*exp(p22)*dw2)
    ## Set the names of the inputs
    model$addInput(Te,Gspl1,Gspl2,Gspl3,Gspl4,Gspl5,Qi,stepQi)
    ...
    model$setParameter(  gA1 = c(init=0.1 ,lb=0.001 ,ub=10))
    model$setParameter(  gA2 = c(init=0.1 ,lb=0.001 ,ub=10))
    model$setParameter(  gA3 = c(init=0.1 ,lb=0.001 ,ub=10))
    model$setParameter(  gA4 = c(init=0.1 ,lb=0.001 ,ub=10))
    model$setParameter(  gA5 = c(init=0.1 ,lb=0.001 ,ub=10))
    model$setParameter(  p  = c(init=0.2 ,lb=0.001 ,ub=1))
