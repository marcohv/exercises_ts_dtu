sdeTiTw_GvninTw_sigmalevels <- function(X, rerun = FALSE){
    ...
    model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw-Ti) + 1/Ci*Qi + Aw*p/Ci*Gv )*dt + (1+(stepQi*sigmalevel))*exp(p11)*dw1)
    model$addSystem(dTw ~ ( 1/(Cw*Riw)*(Ti-Tw) + 1/(Cw*Rwe)*(Te-Tw) + Aw*(1-p)/Cw*Gv + Awall/Cw*Gvn )*dt + (1+(stepQi*sigmalevel))*exp(p22)*dw2)
    ## Set the names of the inputs
    model$addInput(Te,Gv,Gvn,Qi,stepQi)
    ...
    model$setParameter(Awall = c(init=0.2 ,lb=0.001 ,ub=10))
    model$setParameter(   p = c(init=0.2 ,lb=0     ,ub=1 ))
