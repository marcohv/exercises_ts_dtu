...
*The result is just a straight line. These parameters determine how the spline function will look like. By multiplying the base splines to a solar radiation series and using this as input to an ARX model, the soalr absorption coefficient (gA-value) can vary as a smooth funiton of the sun azimuth angle.*
...
```{r q3_qi_states, echo=FALSE, warning=FALSE, results='hide'}
...
model <- ctsm$new()
## Add system equations and thereby also states
model$addSystem(dTi ~ ( 1/(Ci*Riw)*(Tw-Ti) + Aw/Ci*Gv + 1/Ci*Qi )*dt +
                  (1+(stepQi*sigmalevel))*exp(p11)*dw1)
model$addSystem(dTw ~ ( 1/(Cw*Riw)*(Ti-Tw) + 1/(Cw*Rwe)*(Te-Tw) )*dt +
                  (1+(stepQi*sigmalevel))*exp(p22)*dw2)
## Set the names of the inputs
