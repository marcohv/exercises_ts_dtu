...
abline(h = 0)
```
Find the azimuth angles (in radians) that corresponds to the sunrise and sunset, and assign them to azumith_bound <- c(... , ...) below.

```{r q2_angles, echo=FALSE, warning=FALSE, results='hide'}
## Inset the boundary angles (in radians) below.
library(dplyr)
X %>% mutate(sunAz = round(sunAzimuth, 2),
             sunEle = round(sunElevation, 2),
             crossX = diff(sign(sunEle))) %>%
      select(sunAz, sunEle, crossX) %>%
      filter(sunEle == 0)

azumith.bound <- c(1.03, 5.21)      
